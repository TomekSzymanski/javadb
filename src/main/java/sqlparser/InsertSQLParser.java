...
class InsertSQLParser extends AbstractSQLParser<InsertCommand>  {
    ...
    InsertCommand parse(Tokenizer tokenizer) throws SQLParseException {
        InsertCommand ast = new InsertCommand(); // new command must be created for every new parse INSERT requests
        // expect INSERT INTO
        ...
        if (!columnList.isEmpty()) {
            parseColumnListSpecification(ast, columnList);
        }

        expect(tokenizer, VALUES, "Insert statement does not contain VALUES clause");

        parseValuesSpecification(ast, tokenizer);
    ...
    <column_list_specification> ::= [ <left paren> <insert column list> <right paren> ]
     */
    private void parseColumnListSpecification(InsertCommand ast, List<String> columnList) throws SQLParseException {
    ...
    <value_expression_list> ::= <left paren> <value_expr> [ {<comma> <value_expr>} ] <right paren>
     */
    private void parseValuesSpecification(InsertCommand ast, Tokenizer tokenizer) throws SQLParseException { // TODO what if we have comma as float decimal separator?
